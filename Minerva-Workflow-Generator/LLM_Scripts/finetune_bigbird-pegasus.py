...
from transformers import BigBirdPegasusForConditionalGeneration, AutoTokenizer, Seq2SeqTrainer, Seq2SeqTrainingArguments, DataCollatorForSeq2Seq, BitsAndBytesConfig
import pandas as pd
import numpy as np
import random
...
base_model_id = 'google/bigbird-pegasus-large-bigpatent'
finetuned_model_id = "English_To_Actiongraph_BigBirdPegasus"
...
def get_model_and_tokenizer(model_id):
    ...
    model = BigBirdPegasusForConditionalGeneration.from_pretrained(model_id, device_map="auto", attention_type="original_full",)
...
def preprocess_function(raw_data):
    global tokenizer
    model_inputs = tokenizer(raw_data['inputs'], text_target=raw_data['labels'], max_length=512, truncation=True)
    return model_inputs
